...
//	private static final boolean DEBUG = false;
	...
	private final SharedElementCallback mCallback = new SharedElementCallback() {
		...
		public void onMapSharedElements(List<String> names,Map<String, View> sharedElements) {
			if (mTmpReenterState != null) {
				int startingPosition = mTmpReenterState
						.getInt(EXTRA_STARTING_ALBUM_POSITION);
				int currentPosition = mTmpReenterState
						.getInt(EXTRA_CURRENT_ALBUM_POSITION);
				if (startingPosition != currentPosition) {
					// If startingPosition != currentPosition the user must have
					// swiped to a
					// different page in the DetailsActivity. We must update the
					// shared element
					// so that the correct one falls into place.
					String newTransitionName = ALBUM_NAMES[currentPosition];
					View newSharedElement = mRecyclerView
							.findViewWithTag(newTransitionName);
				...
				if (navigationBar != null) {
					...
					sharedElements.put(navigationBar.getTransitionName(),
							navigationBar);
				}
				if (statusBar != null) {
					names.add(statusBar.getTransitionName());
					sharedElements
							.put(statusBar.getTransitionName(), statusBar);
				}
			}
		}
	};
	...
	public void onActivityReenter(int requestCode, Intent data) {
		...
		mRecyclerView.getViewTreeObserver().addOnPreDrawListener(new ViewTreeObserver.OnPreDrawListener() {
			...
			public boolean onPreDraw() {
				...
				// layout here in order to get a smooth transition.
	...
	private class CardAdapter extends RecyclerView.Adapter<CardHolder> {
		...
		public CardHolder onCreateViewHolder(ViewGroup viewGroup, int viewType) {
			return new CardHolder(mInflater.inflate(R.layout.album_image_card,
					viewGroup, false));
		}
